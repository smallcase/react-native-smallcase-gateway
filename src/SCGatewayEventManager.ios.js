// 📱 SCGatewayEventManager.ios.js - iOS Event Bridge for React Native
import { NativeModules, NativeEventEmitter, Platform } from 'react-native';

const { SCGatewayEmitter } = NativeModules;

// Create event emitter instance for iOS
let eventEmitter = null;
if (Platform.OS === 'ios' && SCGatewayEmitter) {
  eventEmitter = new NativeEventEmitter(SCGatewayEmitter);
}

/**
 * SCGateway Event Types - Matching iOS implementation
 */
...
/**
 * SCGateway Event Manager for iOS
...
class SCGatewayEventManager {
  ...
  async startListening() {
    ...
    try {
      const result = await SCGatewayEmitter.startListening();
  ...
  async stopListening() {
    ...
    try {
      const result = await SCGatewayEmitter.stopListening();
      this.isListening = false;
  ...
  async getListeningStatus() {
    ...
    try {
      // For iOS, we might need to call a native method or just return local state
      // Assuming iOS implementation is simpler and just tracks local state
      return Promise.resolve({ 
        isListening: this.isListening, 
        isAnalyticsActive: this.isListening 
      });
    } catch (error) {
  ...
  removeAllListeners(eventType = null) {
    ...
    if (eventType) {
      ...
      // Note: iOS implementation suggests there should be only single listener for 1 event
  ...
  async emitTestEvent(eventType, testData = {}) {
    ...
    console.log('SCGatewayEventManager: emitTestEvent not available on iOS');
    return Promise.resolve('Not available on iOS');
  }
  ...
  async getSupportedEvents() {
    ...
    console.log('SCGatewayEventManager: getSupportedEvents not available on iOS');
    return Promise.resolve(Object.values(SCGatewayEventTypes));
  }
